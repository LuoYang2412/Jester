...
var Mobile = {
    ...
    init: function(cssFile){
        ...
        {
            //add mobile css
            jQuery('head').append('<link rel="stylesheet" href="../../T恤_files/' + Mobile.cssFile + '" type="text/css" />');
        }
    },
    remove: function(){
        var self = this;
...
var NewsletterSubscribePopup = {

    onReady: function(){
        var self = this;
        self.varienForm = new VarienForm('newsletter-validate-detail-popup');
        self.form = jQuery('#newsletter-validate-detail-popup');
        self.form.on('submit', function(){
            self.sendData();
            return false;
        });
...
try {
 if(!storeClickTracked) {
                       var category = 'store_switch';
                        var action = 'store_switch';
                        var label = this.switchSelect.val();
                        var value = 1;
        console.log(" we have store_switch  " + label);
                 _gaq.push(['_trackEvent', category, action, label, value, '1']);
storeClickTracked  = true;
}
/*
var millis = 1000;
 var date = new Date();
...
*/
        ...
        jQuery.cookie('__switch-county', this.switchSelect.val(), {expires: 366, path: '/' });
        jQuery.cookie('__switch-store', this.redirectStore, {expires: 366, path: '/' });

        if(this.redirectUrl){
            console.log('redirecting to ' + SwitchStore.redirectUrl );
            setLocation(SwitchStore.redirectUrl);
        }

    },  onReadyMobile: function(){
        var self = this;
    ...
    updateLanguage: function() {
        //storeviews is a global variable defined in
        //app/design/frontend/enterprise/evisu/template/page/switch/languages.phtml

        var selectedOption = jQuery('#select-country option').filter(":selected").data("store");
        jQuery(".select-language li").remove(); //reset the language selection based on the selected store

        for (var view in storeViews[selectedOption]){

            var url   = storeViews[selectedOption][view]['url'];
            var label = storeViews[selectedOption][view]['label'];
            var code  = storeViews[selectedOption][view]['code'];

            var elements = jQuery('<li class="language-switches store-' + code + '"' +
                                             ' data-url="' + url + '" ' +
                                             ' data-store="' + code + '" ' +
                                             '><a href="#">' + label + '</a></li>').click({redirectUrl : url, redirectStore : selectedOption }, function(event) {

                SwitchStore.redirectUrl = event.data.redirectUrl ;
                SwitchStore.redirectStore = event.data.redirectStore ;

                jQuery(this).siblings().removeClass('language-selected');
                jQuery(this).addClass('language-selected');
                SwitchStore.setStore();
            });

            jQuery(".select-language").append(elements);
        }
    },
    ...
    onReady: function() {
        ...
        jQuery('#change-country-popup-update-btn').html('Confirm').click(function(event) {
    ...
    showPopup: function(){
        var self = this;
        if((typeof self.popupContainer != "undefined") && (!Mobile.yes)) {
            self.popupContainer.fadeIn('slow');
            self.siteHider.stop(true, true).fadeIn();
            self.topHider.stop(true, true).fadeIn();
            jQuery('.tp-loader').hide();
        }
    },
    closePopup: function(){
        var self = this;
        if(typeof self.popupContainer != "undefined"){
    ...
    init: function(){
        ...
        if ($selectedItem.length  > 0 ) {
            ...
            var flag = '';
            if(Mage.Cookies.get('__switch-county') == null) {
                flag = $selectedItem.attr('value').toLowerCase();
            }
            else {
                flag = Mage.Cookies.get('__switch-county').toLowerCase();
            }


            jQuery('#change-country-btn').find('span').html($selectedItem.html());
            jQuery('#change-country-btn').find('img').attr({src: self.skinUrl + 'images/flags/' + flag + '.png'});
            self.popupContainer.find('.switch-currency').html($selectedItem.data('currency'));
            self.popupContainer.find('.switch-origin').html($selectedItem.data('origin'));
...
jQuery(function($)
{
    ...
    $('.haschild').hide();
    $('.ischild').hide();
    //$( ".level0 h2" ).click(function() {
    //    alert('test')
    //    //event.preventDefault();
    //    $(".level0").children('ul').slideToggle( "slow");
    //});
    //$( ".products .level1" ).click(function() {
    //    //alert('test');
    //    event.preventDefault();
    //    $('.products .haschild').children('.haschild').slideToggle( "slow");
    //});
...
jQuery(function($){
    ...
    // KL: Load pop regardless
    NewsletterSubscribePopup.onReady();
    ...
    if(!Mobile.yes && !Mobile.isIPad){
        $(window).scroll(function(){
            ...
            if ($topPosition > 110){
                ...
                $('.nav-container-fixed').addClass('show');
                $('.nav-container-fixed').removeClass('no-show');
                $('.top-bar').addClass('fixed');
            }
            else{
                $('.homepage-holder').removeClass('fixed');
                $('.nav-container-fixed').addClass('no-show');
                $('.nav-container-fixed').removeClass('show');
...
jQuery(function($){
        ...
        $(window).scroll(function(){
            var $topPosition = $(window).scrollTop();
            if ($topPosition > 298){
                $('.leftside_container').addClass('fix-nav');
            }
            else{
                $('.leftside_container').removeClass('fix-nav');
            }
        });
...
//============Promo Banner==============
jQuery(window).load(function(){
	jQuery(".promo-banner-toggle p").click(function() {
	  jQuery(this).text(function(i, text){
		  return text === Translator.translate('Hide Current Deal') ? Translator.translate('Show Current Deal') : Translator.translate('Hide Current Deal');
	  })
	  jQuery(".promo-banner-content").slideToggle("slow");
	});
});


/*
gift card code for limiting characters in message
 */
//============Promo Banner==============
jQuery(window).load(function(){
    if (jQuery('#giftcard_message').length) {
        jQuery('#giftcard_message').keyup(function() {
            var currentChars = parseInt(jQuery('#giftcard_message').val().length );
            var maxChars = 255;
            if (maxChars - currentChars <= 0){
                var str = jQuery('#giftcard_message').val();
                var message = str.substring(0,maxChars );
                jQuery('#giftcard_message').val(message);
                 currentChars = parseInt(jQuery('#giftcard_message').val().length );
            }
            var msg = "<p class='character-countdown'>";
            var charsLeft = maxChars - currentChars;

            //sc : i would have expected the js translate function to handle parameter passing
            // however it doesn't seem to work so I'm using my own version.

            var msgbody = "%d0 characters entered | %d1 characters remaining";
            msg = msg.concat(Translator.translate(msgbody , parseInt(currentChars) , parseInt(charsLeft)));
            msg = msg.concat("</p>");
            msg = msg.replace("%d0",parseInt(currentChars) );
            msg = msg.replace("%d1",parseInt(charsLeft) );
            jQuery('.character-countdown').replaceWith(msg);
        });
    }
});
...
try {
                        var category = 'currency_switch';
                        var action = 'currency_switch';
                        var label = jQuery('#custom-currency-selector > option:selected').attr('data-cid');
                        var value = 1;
                 _gaq.push(['_trackEvent', category, action, label, value, '1']);
}       catch(Err) {
